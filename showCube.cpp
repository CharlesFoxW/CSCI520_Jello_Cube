...
void showCube(struct world * jello)
{
  ...
  /* normals buffer and counter for Gouraud shading*/
      ...
      for (j=0; j<=7; j++)
        for (k=0; k<=7; k++)
        {
          ...
          glBegin(GL_POINTS); // draw point
            glColor4f(0,0,0,0);  
            glVertex3f((GLfloat) jello->p[i][j][k].x, (GLfloat) jello->p[i][j][k].y, (GLfloat) jello->p[i][j][k].z);
          glEnd();
  ...
  else
  {
      ...
      // 1 = bottom, 2 = front, 3 = left, 4 = right, 5 = far, 6 = top
    {
      ...
      /* process triangles, accumulate normals for Gouraud shading */
      ...
      for (i=0; i <= 6; i++)
        ...
        for (j=1; j<=7; j++) 
        {
          ...
          for (i=0; i<=7; i++)
          {
            glNormal3f((GLfloat) (normal[i][j].x / counter[i][j]), (GLfloat) (normal[i][j].y / counter[i][j]),
              (GLfloat) (normal[i][j].z / counter[i][j]));
            glVertex3f((GLfloat) NODE(face,i,j).x, (GLfloat) NODE(face,i,j).y, (GLfloat) NODE(face,i,j).z);
            glNormal3f((GLfloat) (normal[i][j-1].x / counter[i][j-1]),(GLfloat) (normal[i][j-1].y/ counter[i][j-1]),
              (GLfloat) (normal[i][j-1].z / counter[i][j-1]));
            glVertex3f((GLfloat) NODE(face,i,j-1).x, (GLfloat) NODE(face,i,j-1).y, (GLfloat) NODE(face,i,j-1).z);
          }
          glEnd();
        }
